...
// 创建语音引导音频上下文
const guidanceAudio = wx.createInnerAudioContext();
guidanceAudio.loop = false;
...
Page({
  ...
  startMeditation: function() {
    ...
    // 设置主音频
    backgroundAudio.src = SCENE_CONFIG[scene].audio;
    backgroundAudio.volume = this.data.audioSettings.masterVolume;
  ...
  playGuidanceAudio: function(audioPath, index) {
    try {
      ...
      // 设置音频源和音量
      guidanceAudio.src = audioPath;
      guidanceAudio.volume = this.data.audioSettings.guidanceVolume;
  ...
  changeGuidanceSpeed: function(e) {
    ...
    if (this.data.isPlaying && this.data.step === 'meditation') {
      // 暂停当前播放的引导语
      if (guidanceAudio && !guidanceAudio.paused) {
        guidanceAudio.stop();
      }
  ...
  resumeMeditation: function() {
    ...
    // 恢复引导音频播放
    if (guidanceAudio && guidanceAudio.paused && this.data.currentAudioIndex >= 0) {
      guidanceAudio.play();
    }
    ...
    if (this.data.currentAudioIndex < 0 || guidanceAudio.paused) {
      // 从当前进度计算应该播放的引导语
      const elapsed = this.data.elapsedTime;
      const segments = this.data.currentSegments;
