...
const GUIDANCE_TEXTS = [
  "现在，请找到一个舒适的姿势，轻轻闭上眼睛……\n想象你正站在一片开阔的海岸边，阳光温柔地洒在皮肤上，远处传来层层叠叠的海浪声……\n深吸一口气，让空气充满胸腔，再缓缓吐出，仿佛将身体里紧绷的思绪随风带走……\n慢慢的感受 吸气 呼气",
  "望向眼前的大海——它是深邃的蓝，还是泛着银光的灰绿？\n海面上跃动的光斑，是否像星星坠入水中，随着波浪轻轻摇晃？",
  "听……此刻的浪声是轻柔的呢喃，还是有力的低吼？\n试着分辨潮水退去时，砂砾被卷走的细碎声响……'",
  "赤脚感受沙粒的温度，是温热还是微凉？\n当海风掠过发丝，你的额头、手臂或脖颈，哪一处最先感知到它的流动？",
  "如果这片海面是你此刻的心境，它是平静如镜，还是暗流涌动？\n若有心事像礁石沉在海底，你愿意让哪一层波浪托起它，轻轻带向远方……",
  "现在，想象海风穿过你的身体，带走最后一丝疲惫……\n看，沙滩上你写下的一句话，正被潮水温柔抹去，化作泡沫融进大海……\n慢慢将注意力拉回呼吸，感受脚底与地面的接触……\n听着我的声音，从3倒数时，带着海风的余韵睁开双眼：3——身体逐渐苏醒，2——手指微微颤动，1——带着宁静，回到此刻…"
];

// 语音引导音频文件路径
const GUIDANCE_AUDIO_PATHS = [
  '/assets/audio/beach_audio/beach_1.m4a',
  '/assets/audio/beach_audio/beach_2.m4a',
  '/assets/audio/beach_audio/beach_3.m4a',
  '/assets/audio/beach_audio/beach_4.m4a',
  '/assets/audio/beach_audio/beach_5.m4a',
  '/assets/audio/beach_audio/beach_6.m4a'
];
...
Page({
  ...
  onLoad: function () {
    // 设置背景音频
    try {
      backgroundAudio.src = '/assets/audio/beach_background.m4a';
      ...
      backgroundAudio.onError((res) => {
        ...
        // 如果背景音乐加载失败，尝试使用默认的背景音乐
        this.tryFallbackBackgroundAudio();
      });
    } catch (error) {
  ...
  // 尝试使用备用背景音乐
  tryFallbackBackgroundAudio: function() {
    try {
      // 尝试使用森林场景的背景音乐代替
      backgroundAudio.src = '/assets/audio/forest_background.m4a'; 
      backgroundAudio.volume = this.data.backgroundVolume;
      console.log('使用备用背景音乐');
    } catch (error) {
      console.error('设置备用背景音频也失败:', error);
    }
  },
    ...
    voiceGuidanceAudios.forEach((audio, index) => {
      try {
        ...
        audio.onError((res) => {
          ...
          wx.showToast({
            title: `语音引导 ${index + 1} 加载失败`,
            icon: 'none'
          });
          
          // 错误处理 - 尝试使用森林引导代替
          this.tryFallbackGuidanceAudio(index);
        });
      } catch (error) {
  ...
  // 尝试使用备用语音引导
  tryFallbackGuidanceAudio: function(index) {
    try {
      // 尝试使用森林场景的语音引导代替
      const fallbackPath = `/assets/audio/forest_audio/forest_${index + 1}.m4a`;
      voiceGuidanceAudios[index].src = fallbackPath;
      console.log(`使用备用语音引导: ${fallbackPath}`);
    } catch (error) {
      console.error(`设置备用语音引导 ${index + 1} 也失败:`, error);
    }
  },
  ...
  completeMeditation: function () {
    ...
    // 保存冥想记录
    const record = app.saveMeditationRecord('beach', this.data.elapsedTime);
    
    wx.showModal({
      title: '冥想完成',
      content: '恭喜你完成了海滩冥想之旅',
      confirmText: '返回',
      cancelText: '记录感受',
      success: (res) => {
        if (res.confirm) {
